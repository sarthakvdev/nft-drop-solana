...
const CandyMachine = ({ walletAddress }) => {
  ...
  const getProvider = () => {
    const rpcHost = process.env.REACT_APP_SOLANA_RPC_HOST;
    // create a new connection obj
    const connection = new Connection(rpcHost);

    // creating a new solana provider obj
  ...
  const getCandyMachineState = async () => {
    ...
    // get metadeta about your reployed program
    const idl = await Program.fetchIdl(candyMachineProgram, provider);
    ...
    // when we use fetch, it fetches data from devnet to show i client
    const candyMachine = await program.account.candyMachine.fetch(
      process.env.REACT_APP_CANDY_MACHINE_ID
    );

    // Parse out all out metadata and log it out
    const itemsAvailable = candyMachine.data.itemsAvailable.toNumber();
    const itemsRedeemed = candyMachine.itemsRedeemed.toNumber();
    const itemsRemaining = itemsAvailable - itemsRedeemed;
    const goLiveData = candyMachine.data.goLiveDate.toNumber();

    // will use this later in UI
  ...
  const DropTimer = () => {
    ...
    // logic: if currentDate is before dropDate => render
